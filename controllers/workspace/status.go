...
// we keep workspace status failed if it's stopped by workspace startup failure
func changeToStoppedIfNeeded(c *currentStatus) {
	if c.phase != dw.DevWorkspaceStatusFailed {
		c.phase = dw.DevWorkspaceStatusStopped
	}
}
